...
static func _tree(parent: Node, rng: RandomNumberGenerator, b: Dictionary, x: float, z: float, s: float, with_collision: bool) -> void:
	...
	if b.has("tree_models"):
		var models: Array = b.tree_models
		place_prop(parent, models[rng.randi() % models.size()],
...
static func _build_details(parent: Node3D, rng: RandomNumberGenerator, b: Dictionary, biome_id: String, obstacles: Array) -> void:
	# травинки (конусы)
	...
	_scatter(parent, rng, blade, int(b.grass), 3.0, WORLD_RADIUS - 1.5, 0.7, 1.5, 0.15)
	...
	_scatter(parent, rng, bush, 22, 6.0, WORLD_RADIUS - 2.0, 0.6, 1.4, 0.1)
	...
	match biome_id:
		"meadow":
			for fc in [Color(1, 1, 0.85), Color(1.0, 0.8, 0.3), Color(0.95, 0.5, 0.6), Color(0.7, 0.6, 1.0)]:
				...
				flower.material = fm
				_scatter(parent, rng, flower, 26, 3.0, WORLD_RADIUS - 2.0, 0.8, 1.4, 0.28)
		"autumn":
			# опавшие листья — фигурные, с центральной жилкой
			_scatter(parent, rng, leaf_mesh(Color(0.8, 0.45, 0.12)), 260, 3.0, WORLD_RADIUS - 1.0, 1.0, 2.2, 0.01)
			_scatter(parent, rng, leaf_mesh(Color(0.62, 0.25, 0.1)), 200, 3.0, WORLD_RADIUS - 1.0, 1.0, 2.0, 0.01)
			_scatter(parent, rng, leaf_mesh(Color(0.72, 0.55, 0.15)), 150, 3.0, WORLD_RADIUS - 1.0, 1.0, 2.0, 0.01)
		"winter":
			# сугробы
			...
			_scatter(parent, rng, mound, 40, 4.0, WORLD_RADIUS - 1.5, 0.6, 1.8, 0.0, 0.6, obstacles)
		"night":
			# светящиеся грибочки
			for i in 40:
				var a := rng.randf_range(0, TAU)
			...
			for i in 12:
				var a := rng.randf_range(0, TAU)
				var r := rng.randf_range(9.0, WORLD_RADIUS - 4.0)
			...
			for i in 9:
				var a := rng.randf_range(0, TAU)
				var r := rng.randf_range(5.0, WORLD_RADIUS - 3.0)
			...
			for i in 8:
				var a := rng.randf_range(0, TAU)
				var r := rng.randf_range(7.0, WORLD_RADIUS - 4.0)
				var p := Vector3(cos(a) * r, 0, sin(a) * r)
	...
	for i in 6:
		var a := rng.randf_range(0, TAU)
		var r := rng.randf_range(10.0, WORLD_RADIUS - 6.0)
		var x := cos(a) * r
		var z := sin(a) * r
	...
	for i in 5:
		var a := rng.randf_range(0, TAU)
		var r := rng.randf_range(8.0, WORLD_RADIUS - 5.0)
		var x := cos(a) * r
		var z := sin(a) * r
...
static func _ambient_particles(parent: Node3D, kind: String) -> void:
	var p := GPUParticles3D.new()
	var mat := ParticleProcessMaterial.new()
	mat.emission_shape = ParticleProcessMaterial.EMISSION_SHAPE_BOX
	mat.emission_box_extents = Vector3(WORLD_RADIUS, 7, WORLD_RADIUS)
	...
	match kind:
		...
		"leaves":
			...
			p.visibility_aabb = AABB(Vector3(-WORLD_RADIUS - 5, -2, -WORLD_RADIUS - 5), Vector3(WORLD_RADIUS * 2 + 10, 16, WORLD_RADIUS * 2 + 10))
			p.position.y = 4.0
			p.preprocess = 6.0
			parent.add_child(p)
			return
		"fireflies":
			...
			dm.radius = 0.04
			dm.height = 0.08
			mat.emission_box_extents = Vector3(WORLD_RADIUS, 2.5, WORLD_RADIUS)
		_: # pollen
			p.amount = 150
			p.lifetime = 10.0
...
static func build(parent: Node3D, world_seed: int, biome_id: String, pvp := false) -> Dictionary:
	...
	# --- земля (шум делает поверхность неоднородной) ---
	var ground := MeshInstance3D.new()
	var gm := CylinderMesh.new()
	gm.top_radius = WORLD_RADIUS + 40.0
	gm.bottom_radius = WORLD_RADIUS + 40.0
	gm.height = 0.1
	...
	floor_box.size = Vector3(180, 1, 180)
	floor_shape.shape = floor_box
	floor_shape.position.y = -0.5
	floor_body.add_child(floor_shape)
	parent.add_child(floor_body)

	# --- невидимая стена по границе арены ---
	...
	for i in wall_count:
		...
		var shape := CollisionShape3D.new()
		var box := BoxShape3D.new()
		var seg := TAU * (WORLD_RADIUS + 1.0) / wall_count
		box.size = Vector3(seg * 1.2, 6.0, 1.0)
		shape.shape = box
		wall_body.add_child(shape)
		shape.global_position = Vector3(cos(a) * (WORLD_RADIUS + 1.5), 3.0, sin(a) * (WORLD_RADIUS + 1.5))
